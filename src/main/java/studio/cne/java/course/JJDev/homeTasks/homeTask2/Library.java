...
public class Library {
        ...
        for (int i = 0; i < this.books.size(); i++) {
            if (title.equals(this.books.get(i).getTitle())) {
                if (this.books.get(i).isRented() == false) {
                    ...
                    System.out.println("!The book" + this.books.get(i).getTitle() + "is already rented!");
                }
            } else {
                continue;
            }
    ...
    public void rentWithFunction(String title) {

        boolean presence = CommonMethods.checkBookPresence(this, title);
        if (presence) {
            System.out.println("The book: " + title + " is rented to You.");
        } else {
